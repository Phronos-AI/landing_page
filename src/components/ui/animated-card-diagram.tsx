...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-[356px] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-200 dark:bg-white",
        className
      )}
      {...props}
    />
...
const Layer1: React.FC<LayerProps> = ({ hovered, color, secondaryColor }) => {
  ...
  return (
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute top-0 left-0 z-[7] flex h-[360px] w-[356px] transform items-center justify-center transition-transform duration-500 group-hover/animated-card:-translate-y-[90px] group-hover/animated-card:scale-110">
      <div className="relative flex h-[120px] w-[120px] items-center justify-center text-[#00000050] dark:text-black">
        <div className="donut-chart-container relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xl text-black dark:text-black font-semibold">
              {hovered
  ...
  return (
    ...
    >
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[6] flex w-[356px] translate-y-0 items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-full">
        <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] rounded-md border border-zinc-200 bg-white/25 px-2 py-1.5 opacity-100 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-0 dark:border-zinc-200 dark:bg-white/25">
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  const items = [
    { id: 1, translateX: "100", translateY: "50", text: "Python", type: "language" },
    { id: 2, translateX: "100", translateY: "-50", text: "JavaScript", type: "language" },
    { id: 3, translateX: "125", translateY: "0", text: "TypeScript", type: "language" },
    { id: 4, translateX: "-125", translateY: "0", text: "Rust", type: "language" },
    { id: 5, translateX: "-100", translateY: "50", text: "Go", type: "language" },
    { id: 6, translateX: "-100", translateY: "-50", text: "C++", type: "language" },
  ];

  // Alternative items for performance metrics
  const performanceItems = [
    { id: 1, translateX: "100", translateY: "50", text: "Speed", type: "metric" },
    { id: 2, translateX: "100", translateY: "-50", text: "Quality", type: "metric" },
    { id: 3, translateX: "125", translateY: "0", text: "Accuracy", type: "metric" },
    { id: 4, translateX: "-125", translateY: "0", text: "Efficiency", type: "metric" },
    { id: 5, translateX: "-100", translateY: "50", text: "Reliability", type: "metric" },
    { id: 6, translateX: "-100", translateY: "-50", text: "Innovation", type: "metric" },
  ];

  // Determine which items to use based on color (simple way to differentiate)
  const currentItems = color === "#A259FF" ? items : performanceItems;
  ...
  return (
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex items-center justify-center opacity-0 transition-opacity duration-500 group-hover/animated-card:opacity-100">
      {currentItems.map((item, index) => (
        <div
          key={item.id}
          className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute flex items-center justify-center gap-1 rounded-full border border-zinc-200 bg-white/70 px-1.5 py-0.5 backdrop-blur-sm transition-all duration-500 dark:border-zinc-200 dark:bg-white/70"
          style={{
            transform: hovered
